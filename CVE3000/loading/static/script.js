document.addEventListener('DOMContentLoaded', () => {
    ...
    function fetchLogContent() {
        fetch('/log.txt')
            ...
            .then(data => {
                const logs = data.log;  // 假设服务器返回的是一个包含日志数组的 JSON 对象
                const last50Logs = logs.slice(-100);  // 获取最后50行日志
                logContent.innerHTML = '';
                last50Logs.forEach(log => {
    ...
    setInterval(fetchLogContent, 1500);  // 每5秒刷新一次日志内容
    setInterval(fetchProgressContent, 1500);  // 每5秒刷新一次进度内容
});
