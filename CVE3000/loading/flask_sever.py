...
@app.route('/log.txt')
def get_log_file():
    log_content = read_file_with_encoding('log.txt')  # 指定完整的檔路徑
    log_lines = log_content.splitlines() # 僅返回最後25行
    return jsonify({"log": log_lines})

@app.route('/progress.txt')
def get_progress_file():
    return send_from_directory('.', 'progress.txt')
